...
# print(gcs)
# print(sys.argv[6])
tab = tab.rename(columns = {"GC(%)": "GC", 'num_seqs': 'Reads', 'sum_len': 'Yield','min_len':'Min len','max_len':'Max len', 'avg_len':'Avg len', 'AvgQual': 'Qscore'})
dpth,size = get_dpth(genome_size = sys.argv[3], bases = tab['Yield'].values[0])
tab['Depth'] = dpth
tab["Genome size"] = size
# tab['GC'] = gcs
tab['is_control'] = True if "control" in sys.argv[5] else False
tab['filesize'] = "<20000000" if sys.argv[6] == "FAIL_READ_FILE_TOO_SMALL" else ">20000000"
